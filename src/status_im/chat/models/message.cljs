...
(re-frame/reg-fx
 ...
 (fn [{:keys [data-payload tokens]}]
   (let [data-payload-json (types/clj->json data-payload)
     ...
     ;; NOTE: react-native-firebase doesn't have a good implementation of sendMessage (supporting e.g. priority or content_available properties), therefore we must use an implementation in status-go
